...
#if defined DLT_LIB_USE_UNIX_SOCKET_IPC || defined DLT_LIB_USE_VSOCK_IPC || DLT_LIB_USE_LOCAL_SOCKET_IPC
static DltReturnValue dlt_socket_set_nonblock_and_linger(int sockfd)
{
    ...
    l_opt.l_linger = 10;
#ifdef __WIN32__
    if (setsockopt(sockfd, SOL_SOCKET, SO_LINGER, (const char*)&l_opt, sizeof l_opt) < 0)
#else
...
#elif defined DLT_LIB_USE_LOCAL_SOCKET_IPC
static DltReturnValue dlt_initialize_local_socket_connection()
{
	DLT_SEM_LOCK();
	...
	{
		dlt_log(LOG_CRIT, "winsock load failed\n");
        DLT_SEM_FREE();
        return DLT_RETURN_ERROR;
	}
	...
	{
		dlt_log(LOG_CRIT, "socket() failed\n");
        DLT_SEM_FREE();
        return DLT_RETURN_ERROR;
	}
	...
	{
		if (dlt_user.connection_state != DLT_USER_RETRY_CONNECT) {
			dlt_log(LOG_INFO, "connect() failed,Retrying later...\n");
            dlt_user.connection_state = DLT_USER_RETRY_CONNECT;
        }
...
DltReturnValue dlt_init(void)
{
    ...
    */
    ...
    if (dlt_initialize_vsock_connection() != DLT_RETURN_OK)
        return DLT_RETURN_ERROR;
#elif defined DLT_LIB_USE_LOCAL_SOCKET_IPC
	if (dlt_initialize_local_socket_connection() != DLT_RETURN_OK)
        return DLT_RETURN_ERROR;
#elif defined  DLT_LIB_USE_FIFO_IPC 
...
DltReturnValue dlt_free(void)
{
    ...
    if (dlt_user.dlt_log_handle != -1) {
...
#if defined DLT_LIB_USE_UNIX_SOCKET_IPC || defined DLT_LIB_USE_VSOCK_IPC
#ifdef __WIN32__
        ret = shutdown(dlt_user.dlt_log_handle, SD_BOTH);
#else
        ...
        if (ret < 0) {
            ...
            while (1) {
#ifdef __WIN32__
                ret = WSAPoll(nfd, 1, DLT_USER_RECEIVE_MDELAY);
#else
                ...
                if (ret < 0) {
                    ...
                    if (bytes_read >= 0) {
                        if (!bytes_read)
                            break;
#ifdef __WIN32__
                        dlt_vlog(LOG_NOTICE, "[%s] data is still readable... [%lld] bytes read\n",
                                __func__, bytes_read);
#else
        ...
        ret = closesocket(dlt_user.dlt_log_handle);
		
		if ((ret < 0) &&(WSAGetLastError() != 10093)) ret = close(dlt_user.dlt_log_handle);
		else ret=0;
...
void dlt_user_log_reattach_to_daemon(void)
{
    ...
    if (dlt_user.dlt_log_handle < 0) {
        ...
        if (dlt_user.connection_state != DLT_USER_CONNECTED)
            ...
            return;
#elif defined DLT_LIB_USE_LOCAL_SOCKET_IPC
		dlt_initialize_local_socket_connection();
		if (dlt_user.connection_state != DLT_USER_CONNECTED)
            return;
#elif defined DLT_LIB_USE_VSOCK_IPC
