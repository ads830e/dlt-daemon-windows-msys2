...
#ifdef __WIN32__
        if (setsockopt(client->sock, SOL_SOCKET, SO_REUSEADDR, (const char*)&yes, sizeof(yes)) < 0)
#else
...
#ifdef __WIN32__
        if (mreq.imr_multiaddr.s_addr == (u_long)-1)
#else
...
DltReturnValue dlt_client_send_all_trace_status(DltClient *client, uint8_t traceStatus)
{
    ...
    if (req == 0) {
#ifdef __WIN32__
        dlt_vlog(LOG_ERR, "%s: Could not allocate memory %llu\n", __func__, sizeof(DltServiceSetDefaultLogLevel));
#else
