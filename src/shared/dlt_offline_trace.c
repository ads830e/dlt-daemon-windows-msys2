...
static int scandir(const char *dir,struct dirent ***namelist,int (*filter)(const void *b),int (*compare )(const struct dirent **, const struct dirent **) )
{
	filter=filter;
	compare=compare;
	...
	char *filespattern=(char*)malloc(strlen(dir)+1+4+20);
	strcpy(filespattern,dir);
	strcat(filespattern,"\\*.*");

	WIN32_FIND_DATA findata;
    HANDLE hfile;
	
	hfile=FindFirstFile(filespattern,&findata);
    while(hfile!=INVALID_HANDLE_VALUE)
    {
        if(findata.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){}
        else ret++;
		
        if(!FindNextFile(hfile,&findata)) break;
    }
	
	if(ret<=0) return ret;
	
	*namelist=(struct dirent **)malloc(ret*sizeof(struct dirent *));
    ...
    while(hfile!=INVALID_HANDLE_VALUE)
    {
        ...
        {
            //FileName fname=FileName(basedir+"\\"+subdir+"\\"+findata.cFileName);
            
			struct dirent *fileinfo =(struct dirent *)malloc(sizeof(struct dirent));
			
			memset(fileinfo,0,sizeof(struct dirent));
			
			fileinfo->d_ino=0;
			fileinfo->d_reclen=0;
			
			strcpy(fileinfo->d_name,dir);
			strcat(fileinfo->d_name,"\\");
			strcat(fileinfo->d_name,findata.cFileName);
			
			fileinfo->d_namlen=strlen(fileinfo->d_name)+1;
			
			(*namelist)[filei]=fileinfo;
...
#ifdef __WIN32__
    cnt = scandir(path, &files, NULL, NULL);
#else
