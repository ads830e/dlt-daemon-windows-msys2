...
DltReturnValue dlt_file_open(DltFile *file, const char *filename, int verbose)
{
    ...
    if (verbose)
        /* print file length */
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG, "File is %llu bytes long\n", file->file_length);
#else
...
DltReturnValue dlt_file_read(DltFile *file, int verbose)
{
    ...
    if (0 != fseek(file->handle, file->file_position, SEEK_SET)) {
#ifdef __WIN32__
	dlt_vlog(LOG_WARNING, "Seek failed to file_position %llu\n",
                 file->file_position);
#else
...
#ifdef __WIN32__
	dlt_vlog(LOG_INFO, "Position in file: %llu\n", file->file_position);
#else
...
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG, "Position in file: %llu\n", file->file_position);
#else
...
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG,
             "%s: Buffer: Size %u, Start address %llX\n",
             __func__, buf->size, (unsigned long long)buf->mem);
#else
	dlt_vlog(LOG_DEBUG,
...
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG,
             "%s: Buffer: Size increased to %u bytes with start address %llX\n",
             __func__,
             buf->size + (int32_t)sizeof(DltBufferHead),
             (unsigned long long)buf->mem);
#else
	dlt_vlog(LOG_DEBUG,
...
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG,
             "%s: Buffer: Buffer minimized to Size %u bytes with start address %llX\n",
             __func__, buf->size, (unsigned long long)buf->mem);
#else
	dlt_vlog(LOG_DEBUG,
...
#ifdef __WIN32__
	dlt_vlog(LOG_WARNING,
             "%s: Buffer: Buffer reset triggered. Size: %u, Start address: %llX\n",
             __func__, buf->size, (unsigned long long)buf->mem);
#else
	dlt_vlog(LOG_WARNING,
...
#ifdef __WIN32__
	dlt_vlog(LOG_DEBUG,
             "Buffer: Available size: %u, Buffer: Buffer full start address: %llX, Buffer: Buffer start address: %llX\n",
             buf->size, (unsigned long long)buf->shm, (unsigned long long)buf->mem);
#else
	dlt_vlog(LOG_DEBUG,
                                          ...
                                          int __attribute__((unused)) verbose)
{
    ...
    else if ((type_info & DLT_TYPE_INFO_SINT) || (type_info & DLT_TYPE_INFO_UINT))
    {
        ...
        case DLT_TYLE_64BIT:
        {
            if (type_info & DLT_TYPE_INFO_SINT) {
				...
				/*mingw64*/
                snprintf(value_text, textlength, "%lld", value64i);
    #else
	...
	#elif defined (__WIN32__)
                snprintf(value_text, textlength, "%llu", value64u);
    #else
...
int dlt_execute_command(char *filename, char *command, ...)
{
	...
	{
		if(args[argi]==NULL) break;
		cmdfulllen+=strlen(args[argi])+1;//add " "
	}
	if(filename!=NULL)
	{
		cmdfulllen+=2+strlen(filename);
	}
	
	char *cmdfull=(char *)malloc(cmdfulllen*sizeof(char));
	memset(cmdfull,0,cmdfulllen*sizeof(char));
	
	for(int argi=0;argi<argc;argi++)
	{
		if(args[argi]==NULL) break;
		strcat(cmdfull,args[argi]);
		strcat(cmdfull," ");
	}
