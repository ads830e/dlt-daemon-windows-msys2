...
int option_file_parser(DltDaemonLocal *daemon_local)
{
...
#if defined(DLT_DAEMON_USE_UNIX_SOCKET_IPC) || defined(DLT_DAEMON_USE_LOCAL_SOCKET_IPC)
...
#ifdef __WIN32__
        dlt_vlog(LOG_WARNING, "%s: snprintf truncation/error(%lld) %s\n",
	...
	dlt_vlog(LOG_INFO, "%s: Load configuration from file: %s\n",
                __func__, filename);
...
#ifdef __WIN32__
	strcpy(daemon->ECUVersionString,"0");
#else
...
static int dlt_daemon_init_serial(DltDaemonLocal *daemon_local)
{
    ...
    if (isatty(fd)) {
#ifdef __WIN32__
		daemon_local->baudrate = DLT_DAEMON_SERIAL_DEFAULT_BAUDRATE;
		dlt_vlog(LOG_ERR, "Failed to configure serial device\n");
...
//tqtqtqtqtqt
#ifdef __WIN32__
static DltReturnValue dlt_daemon_initialize_control_local_socket(DltDaemonLocal *daemon_local)
{
    int fd = -1;
    ...
    if (head == NULL) { /* no IP set in BindAddress option, will use "0.0.0.0" as default */
        ...
        if (dlt_daemon_socket_open(&fd, 3492, "0.0.0.0") == DLT_RETURN_OK) {
			if (dlt_connection_create(daemon_local,
									  &daemon_local->pEvent,
									  fd,
									  POLLIN,
        ...
        while (head != NULL) { /* open socket for each IP in the bindAddress list */
            ...
            if (dlt_daemon_socket_open(&fd, 3492, head->ip) == DLT_RETURN_OK) {
                if (dlt_connection_create(daemon_local,
										  &daemon_local->pEvent,
										  fd,
										  POLLIN,
...
#ifdef __WIN32__
	if (dlt_daemon_initialize_control_local_socket(daemon_local) < DLT_RETURN_OK) {
        dlt_log(LOG_ERR, "Could not initialize control socket.\n");
        return DLT_RETURN_ERROR;
    }
#else
...
void dlt_daemon_signal_handler(int sig)
{
    ...
    switch (sig) {
...
#endif
        {
            /* finalize the server */
#ifdef __WIN32__
			dlt_vlog(LOG_NOTICE, "Exiting DLT daemon due to signal: %d\n",
                     sig);
#else
    ...
    int verbose)
{
    ...
    if ((in_sock = accept(receiver->fd, (struct sockaddr *)&ctrl, &ctrl_size)) < 0) {
        dlt_vlog(LOG_ERR, "accept() on Local socket %d failed: %s\n", receiver->fd, strerror(errno));
        return -1;
    }
                                                         ...
                                                         int verbose)
{
    ...
    if (application == NULL) {
#ifdef __WIN32__
        dlt_vlog(LOG_WARNING, "Can't add ApplicationID '%.4s' for PID %lld\n",
    ...
    else if (old_pid != application->pid)
    {
        ...
        snprintf(local_str,
                 ...
                 "ApplicationID '%.4s' registered for PID %lld, Description=%s",
                 application->apid,
                 application->pid,
